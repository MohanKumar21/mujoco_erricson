...
class SerializedBuffer:
    def __init__(self, path, device):
        ...
        self.buffer_size = self._n = temp["state"].size(0)
        self.device = device

        self.states = temp["state"].clone().to(device)
        self.actions = temp["action"].clone().to(device)
        self.rewards = temp["reward"].clone().to(device)
        self.next_states = temp["next_state"].clone().to(device)
        self.dones = temp["done"].clone().to(device)
