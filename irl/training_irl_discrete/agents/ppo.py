...
class PPO(nn.Module):
    ...
    def train_network(self, writer, n_epi, states, actions, rewards, next_states, done_masks, old_log_probs):
        ...
        for i in range(self.args.train_epoch):
            for state, action, old_log_prob, advantage, return_, old_value in make_mini_batch(
                self.args.batch_size, states, actions, old_log_probs, advantages, returns, old_values):

                dist = self.get_dist(state)
                value = self.v(state).float()
                curr_log_prob = dist.log_prob(action).unsqueeze(-1)
                entropy = dist.entropy().mean() * self.args.entropy_coef
